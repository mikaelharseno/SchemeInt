...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            current_symbol=formals.first
            current_value=vals.first
            child.define(current_symbol,current_value)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** REPLACE THIS LINE ***"
        target_variable=target.first
        formals=target.second
        body=expressions.second
        env.define(target_variable,LambdaProcedure(formals, body, env))
...
def do_and_form(expressions, env):
    ...
    if expressions==nil:
        ...
        return scheme_eval(expressions.first,env,True)
    examined_non_tail_value=scheme_eval(expressions.first,env)
    if scheme_falsep(examined_non_tail_value):
        return False
...
def do_or_form(expressions, env):
    ...
    if expressions==nil:
        ...
        return scheme_eval(expressions.first,env,True)
    examined_non_tail_value=scheme_eval(expressions.first,env)
    if scheme_truep(examined_non_tail_value):
        return examined_non_tail_value
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            # BEGIN PROBLEM 14
            "*** REPLACE THIS LINE ***"
            sub_expression=clause.second
            if sub_expression==nil:
                return test
            return eval_all(sub_expression,env)
            # END PROBLEM 14
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        current_examined_binding = bindings.first
        check_form(current_examined_binding, 2, 2)

        current_symbol = current_examined_binding.first
        current_value = scheme_eval(current_examined_binding.second.first, env)

        formals = Pair(current_symbol, formals)
        vals = Pair(current_value, vals)
